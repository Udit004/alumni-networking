...
const Network = ({ isDarkMode }) => {
  ...
  const fetchNetworkData = async () => {
    try {
      ...
      if (userData) {
        ...
        // Fetch recommendations - find users with similar industry, skills, or education
        const recommendationsQuery = query(
          collection(db, 'users'),
          where('role', 'in', ['teacher', 'student', 'alumni'])
        );
  ...
  return (
    <div className="network-container">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-6">My Professional Network</h2>
