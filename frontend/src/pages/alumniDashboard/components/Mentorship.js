...
const Mentorship = ({ isDarkMode, API_URL, user, role }) => {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      ...
      // Use standard endpoint pattern with flag to exclude test data
      const endpoint = `${API_URL}/api/mentorship-applications/user/${user.uid}?firebaseUID=${user.uid}&role=${role}&excludeTestData=true`;
  ...
  const handleAcceptApplication = async (application) => {
    try {
      ...
      const endpoint = `${API_URL}/api/mentorship-applications/${application._id}/accept`;
  ...
  const handleRejectApplication = async (application) => {
    try {
      ...
      const token = await user.getIdToken();
      const endpoint = `${API_URL}/api/mentorship-applications/${application._id}/reject`;
      console.log('Using endpoint:', endpoint);
      ...
      // Show success message
      alert(`Application from ${application.name || application.studentName} has been rejected.`);
