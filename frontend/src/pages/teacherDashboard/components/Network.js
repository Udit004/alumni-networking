...
const Network = ({ currentUser, isDarkMode }) => {
  ...
  const fetchNetworkData = async () => {
    try {
      ...
      if (userData) {
        ...
        // Fetch recommendations - find users in the same department or with similar expertise
        const recommendationsQuery = query(
          collection(db, 'users'),
          where('role', 'in', ['teacher', 'alumni'])
        );
        ...
        recommendationsSnapshot.forEach((doc) => {
          ...
          if (
            ...
            // Calculate mutual connections (simplified implementation)
        ...
        recommendationsData.sort((a, b) => b.mutualConnections - a.mutualConnections);
    ...
    <div key={person.id} className="bg-white dark:bg-gray-700 rounded-lg shadow p-4 flex items-start gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-300">
          {person.role === 'student' && `Student, ${person.program || 'Program N/A'}`}
          {person.role === 'alumni' && `Alumni, ${person.designation || 'Position N/A'} at ${person.company || 'Company N/A'}`}
  ...
  return (
    <div className="network-container">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-6">Professional Network</h2>
